 ...
 *
 * Usage:
 *   import * as rbacService from '@common/node/auth/rbac-service.js';
 *   rbacService.setup(knexInstance); // call once at startup via auth setup()
 *
 *   // In createToken — fetch tenant/role/permission data to embed in JWT
 *   const data = await rbacService.getUserTenantsData(userId, user.tenant);
 *
 *   // Management helpers (e.g. admin routes)
 *   await rbacService.assignRole(userId, tenantId, roleId);
 *   await rbacService.revokeRole(userId, tenantId, roleId);
 *   await rbacService.grantPermission(roleId, permissionId);
 *   await rbacService.revokePermission(roleId, permissionId);
 */
...
const getUserTenantsData = async (userId, defaultTenantId) => {
  ...
  try {
    ...
    logger.error({ err, userId }, 'rbac-service: getUserTenantsData failed');
    return null;
  }
};
